...
public class OutboxEventRoute extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        onException(Exception.class)
            ...
            .logStackTrace(true)
            .handled(true)
            .to("log:com.brytech.cart_service.route.OutboxEventRoute?level=ERROR&showException=true")
            .process(exchange -> {
                Outbox outbox = exchange.getIn().getBody(Outbox.class);
                if (outbox != null) {
        ...
        .to("jpa:com.brytech.cart_service.model.Outbox");


        // Main route
        // Poll for Outbox entries with 'Pending' status every 5 seconds
        from("jpa:com.brytech.cart_service.model.Outbox?consumer.query=select o from Outbox o where o.processed = false and o.status = 'Pending'")
